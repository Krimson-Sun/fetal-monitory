...
class Collector:
    ...
    def update_collector(self, new_data: Dict[str, Optional[float]]) -> None:
        ...
        if ('bpm_s' in new_data) and new_data['bpm_s'] is not None:
            self.__collector['bpm'].loc[len(self.__collector['bpm'])] = [new_data['time_sec'], new_data['bpm_s']]
        if ('uc_s' in new_data) and new_data['uc_s'] is not None:
            self.__collector['uterus'].loc[len(self.__collector['uterus'])] = [new_data['time_sec'], new_data['uc_s']]
