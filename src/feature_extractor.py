...
class Preprocessor:
    ...
    def filter_physiological_signals(
        ...
        bpm_data = (
            bpm_data.sort_values("time_sec")
            .drop_duplicates("time_sec")
            .reset_index(drop=True)
        )
        uterus_data = (
            uterus_data.sort_values("time_sec")
            .drop_duplicates("time_sec")
            .reset_index(drop=True)
        )
        ...
        filtered_bpm["time_sec"] = np.arange(
            bpm_data["time_sec"].min(), bpm_data["time_sec"].max(), 1 / fs_estimated
        )
        filtered_bpm["value"] = np.interp(
            filtered_bpm["time_sec"],
            bpm_data["time_sec"].values,
        ...
        filtered_uterus["time_sec"] = np.arange(
            uterus_data["time_sec"].min(),
            uterus_data["time_sec"].max(),
            1 / fs_estimated,
        )
        filtered_uterus["value"] = np.interp(
            filtered_uterus["time_sec"],
            uterus_data["time_sec"].values,
    ...
    def compute_metrics(
        ...
        if summarize:
            ...
            if len(bpm) > last_10_min:
                ...
                baseline = (
                    np.median(bpm) if len(bpm) > 0 else 130.0
                )  # 130 - средняя нормальная ЧСС

            # Вычисляем STV за последние 2 минуты
            last_2_min = int(120 * fs_bpm)
            if len(bpm) > last_2_min:
                stv, stvs, stvs_window_duration = self.__compute_stv(
                    bpm[-last_2_min:], fs_bpm
                )
            elif len(bpm) > 0:
                stv, stvs, stvs_window_duration = self.__compute_stv(bpm, fs_bpm)
            else:
        ...
        late_ratio = (
            late_decelerations / (total_decelerations)
            if total_decelerations > 0
            else 0.0
        )
        ...
        return {
            ...
            "data_points": len(bpm),
            "time_span_sec": (
                (bpm_data["time_sec"].iloc[-1] - bpm_data["time_sec"].iloc[0])
                if len(bpm_data) > 1
                else 0.0
            ),
        }
    ...
    def __compute_stv(
        self, bpm_signal: np.ndarray, fs: float = 4.0
        ...
        rr_intervals = 60000.0 / (
            bpm_signal + 1e-18
        )  # Преобразуем ЧСС в интервалы RR в мс
    ...
    def __compute_ltv(
        ...
        for i in range(0, len(rr_intervals) - window_size + 1, window_size):
            ...
            windows[int(i / window_size)] = np.max(
                rr_intervals[i : i + window_size]
            ) - np.min(rr_intervals[i : i + window_size])

        ltv = (
            np.median(windows) if len(windows) > 0 else 0.0
        )  # Вычисляем медиану по окнам
        window_delay = window_size * 1 / fs  # Вычисляем длительность окна
    ...
    def __is_late(
        ...
        for contraction in contractions:
            ...
            if not (
                (decel_end / fs_bpm < contraction["start"] / fs_uc)
                or (decel_start / fs_bpm > contraction["end"] / fs_uc)
            ):
                # Находим пик маточного сокращения
                contraction_segment = uc_signal[
    ...
    def __compute_trend(
        self, signal: np.ndarray, window: float = 300.0, fs: float = 4.0
    ) -> float:
        """
